...
!
! either SF with id<0 => kinetic feedback
! either SF with id>0 => thermal feedback with/without delayed cooling
!
...
subroutine feedbk(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  SN_BOOST=0.   !! MAX(mass_gmc*2d33/(scale_d*scale_l**3)/mstar,1d0) !! Leo
  ...
  ESN=eps_sn*1d51/(10.*2d33)/scale_v**2
  ...
  ! Leo: seems that we don't need anymore to check if ok below
  ...
  do j=1,np
     !if(ok(j))then  ! Leo: why this test? 
     mloss(j)=0d0
     mzloss(j)=0d0
     ethermal(j)=0d0
     !endif
  ...
  !if(f_w==0)then ! Leo: why this test here?
  do j=1,np
        !if(ok(j))then
        ...
        if(birth_time.lt.(current_time-t0).and.birth_time.ge.(current_time-t0-dteff(j)))then           
           ...
           !idp(ind_part(j))=-idp(ind_part(j))   ! Leo: put id>0 after feedback?
           idp(ind_part(j))=idp(ind_part(j))   ! Leo: id>0
