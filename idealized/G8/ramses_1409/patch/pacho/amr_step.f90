recursive subroutine amr_step(ilevel,icount)
  ...
  !------------------------
  if(ilevel==levelmin)then
     ! modif courty pour eviter de dumper des snapshots a grand z... impose de declarer foutput1
     !if(aexp<0.166)then
     !   if(mod(nstep_coarse,foutput1)==0.or.aexp>=aout(iout).or.t>=tout(iout))then
  ...
  ! Leo: thermal feedback only if f_w =< 0 
  if(hydro.and.star.and.eta_sn>0.and.f_w.le.0.d0)call thermal_feedback(ilevel)
