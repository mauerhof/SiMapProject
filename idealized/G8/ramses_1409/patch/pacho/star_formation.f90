!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
! Leo
! this version does not make use of debris particles 
! but in case of f_w > 0 (kinetic feedback)
!     use id<0 to tag new stellar particles
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
!################################################################
!################################################################
!################################################################
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           T2=T2-T_poly
           !if(T2>2e4)ok(i)=.false.  ! Leo: no Temp criterion
        end do
