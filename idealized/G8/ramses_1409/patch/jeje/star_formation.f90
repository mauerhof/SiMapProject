!JB-
!! Parameters for star formation:
!!  == m_star -> declared in amr_parameters.f90, initialized in read_hydro_params.f90, used in std. version of feedback.f90 (for stochastic boost) but not in our patch. 
!!  == n_star -> declared in amr_parameters.f90, initialized in read_hydro_params.f90, used in 
...
!JB: The whole routine has sense only in 3D... 
#if NDIM==3

  if(numbtot(1,ilevel)==0) return
  if(.not. hydro)return
!JB  if(ndim.ne.3)return
  if(static)return

  if(verbose)write(*,*)' Entering star_formation'

  !JB: move rans to startup routine ... 
  ...
  if(m_star < 0d0)then
     ...
     vol_min=dx_min**ndim
     !-JB
     mstar=n_star/(scale_nH*aexp**3)*vol_min  ! take aexp scaling out (of scale_nH) so that mstar is independent of redshift.
  else
     !JB- assume parameter m_star is in solar masses.
     !JB mstar=m_star*mass_sph
     mstar = m_star * Msun2g / (scale_d * scale_l**3)  ! particle mass in code units. 
     !-JB
  endif

  dstar=mstar/vol_loc

  ! Birth epoch as proper time or not ... 
...
!JB        ! Temperature criterion
!JB        do i=1,ngrid
!JB           T2=uold(ind_cell(i),5)*scale_T2*(gamma-1.0)
!JB           nH=uold(ind_cell(i),1)*scale_nH
!JB           T_poly=T2_star*(nH/nISM)**(g_star-1.0)
!JB           T2=T2-T_poly
!JB           if(T2>2e4)ok(i)=.false. 
!JB        end do
        ...
        do i=1,ngrid
           ...
           if(ok(i))then
              ...
              if(nstar(i)>0)then
                 ntot=ntot+1
!JB                 if(f_w>0)ndebris_tot=ndebris_tot+1
              endif
           endif
        enddo
...
!JB        ! Update linked list for debris
!JB        if(f_w>0)then
!JB           call remove_free(ind_debris,nnew)
!JB           call add_list(ind_debris,ind_grid_new,ok_new,nnew)
!JB        endif
        ...
        do i=1,nnew
           ...
           xp(ind_part(i),1)=x   ! spawn star particle at cell center
           xp(ind_part(i),2)=y
           xp(ind_part(i),3)=z
           vp(ind_part(i),1)=u   ! spawn star particle at cell velocity
           vp(ind_part(i),2)=v
           vp(ind_part(i),3)=w
           if(metal)zp(ind_part(i))=zg  ! Initial star metallicity

!JB           ! Set GMC particle variables
!JB           if(f_w>0)then
!JB              ! Compute GMC mass without more than 50% of gas depletion
!JB              mdebris=min(f_w*n*mstar,0.5*d*vol_loc-n*mstar)
!JB              ! Add supernova ejecta
!JB              mdebris=mdebris+eta_sn*n*mstar
!JB              ! Remove ejecta from the long lived star mass
!JB              mp(ind_part(i))=n*mstar-eta_sn*n*mstar
!JB              ! Set GMC particle variables
!JB              tp(ind_debris(i))=birth_epoch  ! Birth epoch
!JB              mp(ind_debris(i))=mdebris      ! Mass
!JB              levelp(ind_debris(i))=ilevel   ! Level
!JB              idp(ind_debris(i))=-n          ! Number of individual stars
!JB              xp(ind_debris(i),1)=x
!JB              xp(ind_debris(i),2)=y
!JB              xp(ind_debris(i),3)=z
!JB              vp(ind_debris(i),1)=u
!JB              vp(ind_debris(i),2)=v
!JB              vp(ind_debris(i),3)=w
!JB              ! GMC metallicity + yield from ejecta 
!JB              if(metal)zp(ind_debris(i))=zg+eta_sn*yield*(1-zg)*n*mstar/mdebris
!JB           endif
        ...
        do i=1,ngrid
           if(flag2(ind_cell(i))>0)then
              n=flag2(ind_cell(i))
              d=uold(ind_cell(i),1)
!JB              uold(ind_cell(i),1)=max(d-n*dstar*(1.0+f_w),0.5*d)
!JB-
