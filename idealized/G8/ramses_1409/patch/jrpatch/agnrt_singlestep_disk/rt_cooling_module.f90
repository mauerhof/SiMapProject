...
module rt_cooling_module
  ...
  public rt_set_model, rt_solve_cooling, update_UVrates, cmp_chem_eq     &
         ...
         , iP0, iP1, rt_isIR, rt_isOpt, rt_kIR_sc,rt_kIR_abs,rt_kOpt     & !RTpress
         , rt_kUV, igroupIR, igroupOpt, igroupUV                         & !RTpress
         , rt_isIRtrap, iIRtrapVar, iIRpressVar, rt_pressBoost           & !RTpress
         , rt_metal_cooling                                              & !RTpress
         , group_egy_erg, cs_kn, rt_AGN, group_agnegy_frac                 !agnrt
  ...
  integer::iGroupUV=1                                ! First UV group index !RTpress
  integer::iIRtrapVar=1                              ! IRtrap pscalar index !RTpress
  integer::iIRpressVar=1                             ! IRpress scalar index !RTpress
  real(dp)::rt_pressBoost=1d0          ! Boost on RT pressure               !RTpress
  logical::rt_metal_cooling=.true.     ! Use metal cooling?                 !RTpress
  logical::rt_isIR=.false.             ! Using IR scattering on dust?       !RTpress
  logical::rt_isIRtrap=.false.         ! IR trapping in passive scalar?     !RTpress
  logical::rt_isOpt=.false.            ! Using Optical scattering on dust?  !RTpress
  real(dp)::rt_kIR_sc=1d1              ! IR vs dust scattering opacity      !RTpress
  real(dp)::rt_kIR_abs=0d1             ! IR vs dust absorption opacity      !RTpress
  real(dp)::rt_kOpt=1d3                ! Optical vs dust opacity            !RTpress
  real(dp)::rt_kUV=0d0                 ! UV vs dust opacity                 !RTpress
  logical::rt_AGN=.false.              ! Activate radiation from BHs?       !agnrt
  real(dp),dimension(nGroups)::group_agnegy_frac=0 ! Energy fraction (AGN)  !agnrt
  ! Compton heating/cooling energies [erg]:                                 !agnrt
  real(dp),dimension(nGroups)::group_egy_erg                                !agnrt
  ! Klein-Nishina c-section for photons with energy less than ~10^4 eV:     !agnrt
  real(dp)::cs_kn=6.6d-25              ! [cm-2]                             !agnrt
  ! Compton heating rate                                                    !agnrt
  real(dp),dimension(nGroups)::hRate_Compton ! [erg s-1 cm3]                !agnrt
  real(dp)::four_kT                                                         !agnrt
  real(dp),parameter::mcsq=511d3*ev_to_erg                                  !agnrt
...
!-------------------------------------------------------------------------
  ...
  if (rt_isIR)  iGroupUV=iGroupUV+1      ! First group index for UV photons  !RTpress
  if (rt_isOpt) iGroupUV=iGroupUV+1      ! First group index for UV photons  !RTpress
  if(myid==1) write(*,*) 'IR group index has been set to ',iGroupIR          !RTpress
  if(myid==1) write(*,*) 'Opt group index has been set to ',iGroupOpt        !RTpress
  if(myid==1) write(*,*) 'UV group index has been set to ',iGroupUV          !RTpress
...
!-------------------------------------------------------------------------
  ...
  !DEBUGGING STUFF:
  !real(dp),dimension(1:nvector, n_U, 5000),save:: History   
  !real(dp),dimension(1:nvector,5000),save::dts
  !integer,dimension(1:nvector,5000),save::codes
  !integer::j,lastj
  !END DEBUGGING STUFF
!-------------------------------------------------------------------------
...
!        history(i,:,loopcnt)=U(i,:)!+dU                                     !DEBUG
!        dts(i,loopcnt)=ddt(i)                                              !DEBUG
!        codes(i,loopcnt)=code                                              !DEBUG
        if(.not. dt_ok) then  
...
!        ! START DEBUG-----------------------------------------------------------
!        if(loopcnt .gt.4000 .and. myid==1) then ! print out history and quit ramses
!           lastj=1
!           do j=1,loopcnt
!              dU=history(i,:,j)
!              write(*,900) j,dt/dts(i,j),codes(i,j)
!              write(*,901) history(i,1:10,lastj)
!              write(*,902) dU(1:10)
!              write(*,903) abs(dU(1:10)-history(i,1:10,lastj))/history(i,1:10,lastj)
!              if(codes(i,j).eq.0) lastj=j ! Index of last updated U
!           end do
!           STOP
!        endif
900 format (I4, '  ddt=', 1pe12.3, ' code=', I2,'----------------------------------------------------------')
901 format ('                   U      =', 20(1pe12.3))
902 format ('                   Un     =', 20(1pe12.3))
903 format ('                   U%     =', 20(F12.6))
  ! END DEBUG-----------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real(dp):: rho, rdSc_IR, rdAbs_IR, rdAbs_Opt, rdAbs_UV                    !RTpress
  real(dp):: grAbs                                                          !agnrt
...
!-------------------------------------------------------------------------
  ...
  neInit=ne
  rho = nH / X * mH                                                         !RTpress
  rdSc_IR  = rho*Zsolar*rt_kIR_sc*rt_c_cgs ! IR-dust scatter rate [s-1]     !RTpress
  rdAbs_IR = rho*Zsolar*rt_kIR_abs*rt_c_cgs! IR-dust absorption rate [s-1]  !RTpress
  rdAbs_Opt= rho*Zsolar*rt_kOpt*rt_c_cgs  ! Opt-dust absorption rate [s-1]  !RTpress
  rdAbs_UV = rho*Zsolar*rt_kUV*rt_c_cgs   ! UV-dust absorption rate [s-1]   !RTpress
  ...
  if(rt) then 
     ...
     ! IR, optical and UV depletion by dust absorption: ------------------  !RTpress
     if(rt_isIR) then !IR scattering/abs on dust (abs after T update)       !RTpress
        phSc(iGroupIR)  = phSc(iGroupIR)  + rdSc_IR + rdAbs_IR              !RTpress
     endif                                                                  !RTpress
     if(rt_isOpt) & ! Always deplete these photons, since they go into IR   !RTpress
          phAbs(iGroupOpt) = phAbs(iGroupOpt) + rdAbs_Opt                   !RTpress
     ! UV depletion on dust (also can go into IR):                          !RTpress
     phAbs(iGroupUV:nGroups) = phAbs(iGroupUV:nGroups) + rdAbs_UV           !RTpress

     ! Note to joki (from joki) for the Compton scattering, the flux rate
     ! of change should be the scattering rate, but the photon density
     ! rate of change should correspond to the energy loss/gain.
     ! In this way we model scattering in all directions - 
     ! otherwise the scattered photons always maintain their direction.
     !BEGIN Compton scattering--------------------------------------------  !agnrt
     if(cs_kn .gt. 0d0) then                                                !agnrt
        ! Compton (photon-electron) scatterings                             !agnrt
        four_kT = 4d0 * kb * TK                                             !agnrt
        do i=1,nGroups                                                      !agnrt
           grAbs=0d0                                                        !agnrt
           hRate_compton(i) = rt_c_cgs * cs_kn * group_egy_erg(i)/mcsq    & !agnrt
                * (group_egy_erg(i)-four_kT)                                !agnrt
           if(hRate_compton(i) .lt. 0) then ! Compt. cooling -> add photons !agnrt
              recRad(i) = recRad(i) &                                       !agnrt
                   + hRate_Compton(i) * ne * dU(iNpU(i)) / group_egy_erg(i) !agnrt
           else                        ! Compton heating -> remove photons  !agnrt
              grAbs = hRate_compton(i) * ne / group_egy_erg(i)              !agnrt
              phAbs(i) = phAbs(i) + grAbs                                   !agnrt
           endif                                                            !agnrt
           ! Scattering: Subtr. phAbs bc included in photon flux abs.       !agnrt
           phSc(i) = phSc(i) + MAX(0D0, rt_c_cgs * cs_kn * ne - grAbs )     !agnrt
        end do                                                              !agnrt
     endif ! if(cs_kn .gt. 0)                                               !agnrt
     !END Compton scattering----------------------------------------------  !agnrt
     do i=1,nGroups         ! ------------------- Do the update of N and F
        ...
        if(rt_isIR .and. i .eq. iGroupIR) then ! Use flux for IR            !RTpress
           dU(iP0+i-1) = dU(iP0+i-1) + dU(iFpu(i))/rt_c_cgs * dt         &  !RTpress
                * (phAbs(i)+phSc(i))  * group_egy(i) * ev_to_erg/c_cgs      !RTpress
        else ! Assume f=1, for other groups, where f is reduced flux.       !RTpress
           dU(iP0+i-1) = dU(iP0+i-1) + dU(iNpu(i)) * dt                  &  !RTpress
                * (phAbs(i)+phSc(i))  * group_egy(i) * ev_to_erg/c_cgs      !RTpress
        endif                                                               !RTpress
        ! ----------------------------------------------------------------  !RTpress
     end do
     ...
     fracMax=dUU  ! To check if timestep size can be increased
     dUU=MAXVAL(                                                         &
     ...
     ! -------------------------------------------------------------------  !RTpress
     ! UV->IR; Add absorbed UV on dust energy to the pool of IR photons:    !RTpress
     if(rt_isIR) then                                                       !RTpress
        dU(iNpU(iGroupIR)) = dU(iNpU(iGroupIR)) + rdAbs_UV * dt          &  !RTpress
             * SUM( dU(iNpU(iGroupUV:nGroups))                           &  !RTpress
                    * group_egy(iGroupUV:nGroups) / group_egy(iGroupIR) )   !RTpress
        dU(iNpU(iGroupIR)) = MAX(smallNp, dU(iNpU(iGroupIR)))               !RTpress
     endif                                                                  !RTpress
     ! Opt->IR; Add absorbed optical energy to the pool of IR photons:      !RTpress
     if(rt_isIR .and. rt_isOpt) then                                        !RTpress
        dU(iNpU(iGroupIR)) = dU(iNpU(iGroupIR)) + rdAbs_Opt * dt         &  !RTpress
             *dU(iNpU(iGroupOpt))*group_egy(iGroupOpt)/group_egy(iGroupIR)  !RTpress
        dU(iNpU(iGroupIR)) = MAX(smallNp, dU(iNpU(iGroupIR)))               !RTpress
     endif                                                                  !RTpress
     ! -------------------------------------------------------------------  !RTpress
  endif !if(rt)
  ...
  if(c_switch .and. cooling) then
     ...
     ! Compton heating/cooling:                                             !agnrt
     if(cs_kn .gt. 0d0) then                                                !agnrt
        do i=1,nGroups                                                      !agnrt
           Hrate = Hrate + dU(iNpU(i)) * ne * hRate_Compton(i)              !agnrt
        end do                                                              !agnrt
     endif                                                                  !agnrt
...
!------------------------------------------------------------------------
  ...
  do iP=1,nGroups
     group_egy_erg(iP) = ev_to_erg * group_egy(iP)                         !agnrt
