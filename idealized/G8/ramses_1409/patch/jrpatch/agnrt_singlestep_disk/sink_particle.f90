...
subroutine make_sink(ilevel)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim  
        ...
        do i=1,ngrid
           !d=uold(ind_cell(i),1)
           d=max(uold(ind_cell(i),1),1d-10) !joki
	...
	! Gather cells with a new sink
        ...
        do i=1,ngrid
           if (flag2(ind_cell(i))>0)then
              ...
              dens_tmp(izero_myid+ninc)=max(uold(ind_cell(i),1),1d-10) !joki
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           d=max(uold(ind_cell_new(i),1),1d-10) !joki
        ...
        do ii=-8,8
           ...
           if(rr>0.and.rr<=rmax)then
              ...
              do j=1,np
                 ...
                 levelp(ind_cloud(j))=levelmin !joki
...
subroutine bondi_velocity(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  do j=1,np
     if(ok(j))then
        !d=uold(indp(j),1)
        d=max(uold(indp(j),1),1d-10) !joki
        u=uold(indp(j),2)/d
        v=uold(indp(j),3)/d
        w=uold(indp(j),4)/d
        e=uold(indp(j),5)/d
 ...
 ! Compute parent cell position
  ...
  do ind=1,twotondim
     do j=1,np
        ...
        dgas(j)=dgas(j)+d*vol(j,ind)            !joki
...
subroutine AGN_feedback
  ...
  if(rt_AGN)return     ! WARNING: remove this line if you want both 
                       ! radiation and mechanical feeback

!  ! WARNING: remove this part if you want both 
!  ! radiation and mechanical feeback
!  if(rt_AGN)then
!     do isink=1,nsink
!        dMsmbh_coarse(isink)=0d0
!     enddo
!     return
!  endif
                    ...
                    ! ------------------------------------------                    
                          ...
                          if(ok_blast_agn(iAGN))then
                             ...
                             if(jtot.gt.0d0)then
                                ...
                                if(dr_cell.le.dx_loc/2.0)then
                                   ...
                                   d=MAX(uold(ind_blast(iAGN),1),1d-10) !joki
                       ...
                       ! ------------------------------------------                    
                          ...
                          if(dr_AGN.le.rmax2)then
                             ...
                             mass_gas(iAGN)=mass_gas(iAGN)+vol_loc*max(uold(ind_cell(i),1),1d-10)!joki
                          endif
                          dr_cell=MAX(ABS(dxx),ABS(dyy),ABS(dzz))
                          if(dr_cell.le.dx_loc/2.0)then
                             ind_blast (iAGN)=ind_cell(i)
                             ...
                             mass_blast(iAGN)=vol_loc*max(uold(ind_cell(i),1),1d-10) !joki
                          endif
                       endif
                    endif
                          ...
                          if(jtot.gt.0d0)then
                             ...
                             if (drjet .le. rmax .and. abs(dzjet) .le. rmax)then
                                ...
                                do idim=1,ndim
                                   ...
                                   ekk=ekk+0.5d0*uold(ind_cell(i),idim+1)**2/max(uold(ind_cell(i),1),1d-10) !joki
                                end do
                                ekkold=ekk
                                ...
                                T2_1=(gamma-1d0)*eint/max(uold(ind_cell(i),1),1d-10)*scale_T2 !joki
