subroutine init_part
  ...
  !------------------------------------------------------------
  integer::npart2,ndim2,ncpu2,isink
!  integer::npart2,ndim2,ncpu2,ichem,isink
  ...
  if(star.or.sink)then
     ...
     if(metal)then
        ...
        zp=0.0
        if(chemo)then
           allocate(oxp(npartmax))
           allocate(fep(npartmax))
           oxp=0.0
           fep=0.0
        endif
     end if
  ...
  if(sink)then
     ...
     allocate(dMsmbh_coarse_all(1:nsinkmax))
        ...
        do while (.not.eof)
           ...
           if(myid==1)then
              ...
              do i=1,nvector
                 ...
                 mm(i  ) = mm1
                 ii(i  ) = indglob
                 xx(i,:) = xx(i,:)+gal_center1+boxlen/2d0 !joki
              end do
100           continue
           ...
           do i=1,jpart
              ...
              if(cc(i)==myid)then
                 ...
                 if (minval(xx(i,1:3)) .gt. 0d0 .and. maxval(xx(i,1:3)) .lt. boxlen) then
                     ipart=ipart+1
                     !xp(ipart,1:3)=xx(i,1:3) + gal_center1 + boxlen/2.0D0
                     xp(ipart,1:3)=xx(i,1:3) !joki
     ...
     if(TRIM(initfile(levelmin)).NE.' ')then
        ...
        do i=1,jpart
           ...
           if(cc(i)==myid)then
              ...
              isink=isink+1
              ipart=npart+1
              !write(*,*)'myid,ipart,jpart,npart,isink=',myid,ipart,jpart,npart,isink
              if(ipart>npartmax)then
                 write(*,*)'Maximum number of particles incorrect'
                 write(*,*)'npartmax should be greater than',ipart
                 call clean_stop
