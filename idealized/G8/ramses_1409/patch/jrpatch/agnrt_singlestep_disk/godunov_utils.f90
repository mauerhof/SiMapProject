...
subroutine cmpdt(uu,gg,dx,dt,ncell)
  ...
  use const
  use rt_cooling_module,only:rt_isIRtrap,iIRpressVar                          !RTPress
  ...
  if(rt_isIRtrap .and. rt_advect) then                                !RTpress
     do k=1,ncell                                                     !RTpress
        uu(k,ndim+2) = uu(k,ndim+2) + uu(k,iIRPressVar)               !RTpress
     end do                                                           !RTpress
  endif                                                               !RTpress
...
subroutine riemann_hllc(qleft,qright,qgdnv,fgdnv,ngrid)
  ...
  do i=1,ngrid
     ...
     if(rt_isIRtrap .and. rt_advect) then                                   !RTpress
        Pl = Pl + MAX(qleft(i,iIRpressVar)*rl,0d0)                          !RTpress
     endif                                                                  !RTpress
     ...
     ! Add isotropic radiation pressure                                     !RTpress
     if(rt_isIRtrap .and. rt_advect) then                                   !RTpress
        Pr = Pr + MAX(qright(i,iIRpressVar)*rr,0d0)                         !RTpress
     endif                                                                  !RTpress
