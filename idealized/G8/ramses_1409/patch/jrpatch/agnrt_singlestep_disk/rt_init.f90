...
!-------------------------------------------------------------------------
  ...
  if(rt_isIRtrap) then                                                      !RTpress
     iIRtrapVar  = nvar_count+1                                             !RTpress
     iIRpressVar = nvar_count+2                                             !RTpress
     nvar_count  = nvar_count+2                                             !RTpress
  endif                                                                     !RTpress
  ...
  if(rt_isIRtrap) then                                                      !RTpress
     uold(:,iIRtrapVar)=0d0                                                 !RTpress
     uold(:,iIRpressVar)=0d0                                                !RTpress
  endif                                                                     !RTpress
  ...
  if(rt .and. rt_AGN) rt_advect=.true.                 ! Make sure photons are advected
...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,rt_isIR, rt_isOpt, rt_kIR_abs, rt_kIR_sc, rt_kOpt, rt_kUV      &  !RTpress
       & ,rt_pressBoost, rt_isIRtrap, rt_metal_cooling                   &  !RTpress
       & ,cs_kn, rt_AGN                                                     !AGNRT
...
!-------------------------------------------------------------------------
  ...
  use rt_parameters
  use SED_module
  use rt_cooling_module, only: group_agnegy_frac
  implicit none
  logical::nml_ok
  integer::i
!------------------------------------------------------------------------
...
!                   + rt_n_source(k)/scale_Np/128d0 / dx_cgs**ndim * dt_cgs
