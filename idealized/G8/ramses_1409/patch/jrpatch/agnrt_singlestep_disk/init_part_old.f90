subroutine init_part
  ...
  !------------------------------------------------------------
  integer::npart2,ndim2,ncpu2,ichem,isink
  ...
  if(lightcone)allocate(fp(npartmax,ndim))
  xp=0.0; vp=0.0; fp=0.0; mp=0.0; levelp=0; idp=0
  ...
  if(nrestart>0)then
     ...
     if(star.or.sink)then
        ...
        if(lightcone)then
           ! Read force on particles
           do idim=1,ndim
              read(ilun)xdp
              fp(1:npart2,idim)=xdp
           end do
        end if
        deallocate(xdp)
     end if
     ...
     if(.not. cosmo .and. filetype.ne.'ascii_sink')filetype_loc='ascii'
        ...
        do while (.not.eof)
           ...
           if(myid==1)then
              ...
              do i=1,nvector
                 ...
                 jpart=jpart+1
                 indglob=indglob+1
                 xx(i,1)=xx1+boxlen/2d0
                 xx(i,2)=xx2+boxlen/2d0
                 xx(i,3)=xx3+boxlen/2d0
                 vv(i,1)=vv1
                 vv(i,2)=vv2
                 vv(i,3)=vv3
                 mm(i  )=mm1
...
100           continue
  ...
  if(filetype_sink=='ascii_sink')then

     if(verbose)write(*,*)'Entering ascii_sink'
     ...
     if(TRIM(initfile(levelmin)).NE.' ')then
        ...
        xx=0.0
        if(myid==1)then
           write(*,*)'Creating the sink particle'
           i=1
           jpart=1
           indglob=indglob+1
           xx(i,1)=boxlen/2.0
           ...
           vv(i,2)=0.0
           vv(i,3)=0.0
           mm(i  )=1d7*2d33/(scale_d*scale_l**3d0)
           ii(i  )=indglob
        endif
        buf_count=nvector*3
#ifndef WITHOUTMPI
        ...
        do i=1,jpart
           ...
           if(cc(i)==myid)then
              ...
              levelp(ipart)=levelmin
              idp(ipart)   =ii(i)
!!$              write(*,*)'xp=',xp(ipart,1:3)
!!$              write(*,*)'vp=',vp(ipart,1:3)
!!$              write(*,*)'mp=',mp(ipart)
!!$              write(*,*)'levelp=',levelp(ipart)
!!$              write(*,*)'idp   =',idp(ipart)
!!$              write(*,*)'nsink=',isink,'myid=',myid
#ifndef WITHOUTMPI
           else
              ipart=npart
           endif
     ...
     npart=ipart
     write(*,*)'npart=',npart,'myid=',myid
     
     ! Compute total number of particle
     npart_cpu=0; npart_all=0
     npart_cpu(myid)=isink
#ifndef WITHOUTMPI
     call MPI_ALLREDUCE(npart_cpu,npart_all,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
     npart_cpu(1)=npart_all(1)
#endif
     ...
     write(*,*)'nsink=',isink,'/',npart_cpu(ncpu)
...
!!$     ilun=4*ncpu+10
!!$     filename='mergers_sink.txt'
!!$     open(unit=ilun,file=TRIM(filename),form='formatted',status='unknown',position='append')
!!$     write(ilun,*)'aexp   nstep  id1  id2  igrp'
!!$     do i=1,nsink
!!$        ilun=4*ncpu+10+idsink(i) 
!!$        call title(idsink(i),nchar)
!!$        filename='sink_'//TRIM(nchar)//'.txt'
!!$        open(unit=ilun,file=TRIM(filename),form='formatted',status='unknown',position='append')
!!$        write(ilun,*)'#1      aexp'
!!$        write(ilun,*)'#2      nstep'
!!$        write(ilun,*)'#3      BH mass [Msun]'
!!$        write(ilun,*)'#4-6    position [pc]'
!!$        write(ilun,*)'#7-9    velocity [km/s]'
!!$        write(ilun,*)'#10     mean density [H/cc]'
!!$        write(ilun,*)'#11     mean sound speed [km/s]'
!!$        write(ilun,*)'#12     relative velocity [km/s]' 
!!$        write(ilun,*)'#13-15  jet axis [arbitrary]'
!!$        write(ilun,*)'#16     accretion rate (before the Eddington limit cut) [Msun/yr]'
!!$        write(ilun,*)'#17     Eddington accretion rate [Msun/yr]'
!!$     enddo
!!$  endif
