...
subroutine move1(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  real(dp)::alpha,mach,d_star,factor,fudge,twopi,vnorm_rel,xx
 ...
 ! Compute parent cell position
  ...
  if(sink.and.drag)then
     v_cloud(1:np,1:ndim)=0.0D0
     c_cloud(1:np)=0.0D0
     d_cloud(1:np)=0.0D0
     m_cloud(1:np)=0.0D0
     do ind=1,twotondim
        do j=1,np
           ...
           if(isink>0.and.tp(ind_part(j)).eq.0d0)then
              !d=uold(indp(j,ind),1)
              d=MAX(uold(indp(j,ind),1),1d-10) !joki
              u=uold(indp(j,ind),2)/d
              v=uold(indp(j,ind),3)/d
              w=uold(indp(j,ind),4)/d
              ekk=0.5d0*d*(u*u+v*v+w*w)
              e=uold(indp(j,ind),5)-ekk  
              c_cloud(j)=c_cloud(j)+e*vol(j,ind)
              do idim=1,ndim
                 v_cloud(j,idim)=v_cloud(j,idim)+uold(indp(j,ind),idim+1)*vol(j,ind)
              enddo
              d_cloud(j)=d_cloud(j)+d**2*vol(j,ind)
              m_cloud(j)=m_cloud(j)+d   *vol(j,ind)
           endif
        end do
     end do
     do j=1,np
        isink=-idp(ind_part(j))
        if(isink>0.and.tp(ind_part(j)).eq.0d0)then
           d_cloud(j)=d_cloud(j)/m_cloud(j)
           do idim=1,ndim
              v_cloud(j,idim)=v_cloud(j,idim)/m_cloud(j)
           enddo
           c_cloud(j)=MAX(c_cloud(j),0d0) !joki
           if(m_cloud(j).le. 0d0) print*,'Move_fine->m_cloud ',myid,d_cloud(j),c_cloud(j),m_cloud(j),gamma
           if(c_cloud(j).le. 0d0) print*,'Move_fine->c_cloud ',myid,d_cloud(j),c_cloud(j),m_cloud(j),gamma
           c_cloud(j)=max(sqrt(gamma*(gamma-1d0)*c_cloud(j)/m_cloud(j)),1d-10)!joki
        endif
     enddo
  endif
     ...
     do j=1,np
        ...
        if(isink>0.and.tp(ind_part(j)).eq.0d0)then
           ...
           if(drag)then
              ...
              do idim=1,ndim
                 vrel(idim)=new_vp(j,idim)-v_cloud(j,idim)
              enddo
              vnorm_rel=sqrt( vrel(1)**2 + vrel(2)**2 + vrel(3)**2 )
              mach=vnorm_rel/c_cloud(j)
              alpha=max((d_cloud(j)/d_star)**boost,1d0)
              !factor=alpha * fudge * d_cloud(j)*msink(isink)/c_cloud(j)**2 * min(mach,2d0) / vnorm_rel
              factor=alpha * fudge * d_cloud(j)*msink(isink)/c_cloud(j)**2 / vnorm_rel
              ...
              factor=MIN(factor,1.0d0/dtnew(ilevel))
              do idim=1,ndim
                 fdrag(idim)= - factor * vrel(idim)
              enddo
              ! Compute new sink velocity due to the drag force
              do idim=1,ndim
                 new_vdrag(j,idim)=new_vp(j,idim)+fdrag(idim)*dtnew(ilevel)
              enddo
              do idim=1,ndim
                 new_vp(j,idim)=new_vdrag(j,idim)
              enddo
           endif
