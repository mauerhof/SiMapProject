...
subroutine cic_amr(ind_cell,ind_part,ind_grid_part,x0,ng,np,ilevel)
  ...
  do ind=1,twotondim
     ...
     if(cic_levelmax==0.or.ilevel<cic_levelmax)then
        do j=1,np
           if(ok(j).and.(ttt(j).eq.0.0.and.iii(j).lt.0))then ! YD: Enforce maximum level of refinement for cloud particles
              phi(indp(j,ind))=phi(indp(j,ind))+2*m_refine(ilevel)
           else
              if(ok(j))then
