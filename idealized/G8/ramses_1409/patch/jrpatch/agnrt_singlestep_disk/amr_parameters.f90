module amr_parameters
  ...
  logical::rt      =.false.   ! Radiative transfer activated                    !--add
  ! Sink angular momentum is tracked (and must be read from input at restart)   !--add
  ...
  logical::gas_analytics=.false.  ! Turn on a routine that outputs some gas-related data at every output                                                                  !--add
  logical::aton=.false.       ! Enable ATON coarse grid radiation transfer     !--add
  ...
  real(dp)::texp   =0.0D0     ! Current proper time                                 !--add
  real(dp)::n_sink =1D30      ! Sink particle density threshold in H/cc             !--add
  real(dp)::m_star =-1.0      ! Star particle mass in units of mass_sph             !--add
  ...
  real(dp)::jeans_ncells=-1   ! Jeans polytropic EOS                                !--add
  ...
  real(dp)::mass_gmc=-1.0     ! Stochastic exploding GMC mass                      !--add
  ...
  real(dp)::boost  =2.0d0     ! Boost power factor of the accretion rate
  real(dp)::r_gal  =1.0d2     ! SMBH sphere radius of influence in kpc
  real(dp)::sigmav_max=10d0   ! Maximum relative velocity in the Bondi accretion rate in kpc
  ...
  real(dp)::t_delcool=1.0d1   ! Cooling time-scale of turb energy
  real(dp)::sig_delcool=1d-3  ! Ratio of turb to thermal energy
  ...
  real(dp)::rsink_max=10      ! Sink isolation criterion in kpc                   !--add
  real(dp)::msink_max=1d5     ! Maximum seed mass in solar masses                 !--add
  logical ::self_shielding=.false.                                                !--add
  real(dp)::del_Jeans=1d20    ! Baryon overdensity over which 
  real(dp)::t_ctw  =0.0D0     ! Time at which continuous thermal wind is turned on
  real(dp)::A_snIa =0.0D0     ! Fraction of binary system that gives rise to SNIa
  real(dp)::e_HN   =0.0D0     ! Additional energy from the typical hypernova in 1d51 erg
  real(dp)::eff_sfbk=1d0      ! Efficiency of stellar feedback 
  real(dp)::T2max_fix=-1D0    ! Maximum temperature set by user
  real(dp)::star_ratio_floor=0.25d0
  ! Jeans length criterion is tested (for cosmo runs only)
  ...
  logical ::smbh=.false.               !--add
  logical ::agn=.false.                !--add
  logical ::use_proper_time=.false.    !--add
  logical ::random_jet=.false.
  logical ::blazar=.false.
  logical ::chemo=.false.
  logical ::pseudo_sedov=.false.
  logical ::stellar_wind=.false.     ! Stellar wind switch
  logical ::drag=.false.
  ...
  ! Parameters for better control over the zoom region
  integer::level_zoom=10000    ! Level of the zoom region (useful if using some ivar_refine)
  real(dp)::z_init_zoom=0d0    ! Metallicity of the gas in the zoom region
  ...
  character(LEN=20)::filetype_sink='none'
