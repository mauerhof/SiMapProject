...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  use rt_cooling_module, only: n_U,iNpU,iFpU, rt_solve_cooling,iP0,iP1   &
       ,iPtot,rt_isoPress,iRTisoPressVar,rt_prBoost                          !RTpress
#endif
  implicit none
  ...
  do ind=1,twotondim
     ...
     if(neq_chem) then
        ...
        do ig=1,nGroups
           do i=1,nleaf
              ...
              !U(i,iFpu(ig)) = min(U(i,iNpU(ig))*rt_c_cgs, U(i,iFpu(ig)))                 !JOKI!!!!
           enddo
           if(rt_smooth) then                           ! Smooth RT update
              do i=1,nleaf !Calc addition per sec to Np, Fp for current dt
                 ...
                 !Fpnew = min(Npnew*rt_c_cgs, Fpnew)                                      !JOKI!!!
     ...
     uold(ind_leaf(1:nleaf),ndim+2)=ekk(1:nleaf)+eTherm(1:nleaf)            !RTpress
                                                                            !RTpress
     ! Isotropic photon pressure =========================================  !RTpress
