  ...
  if(sink.and.drag)then
     ...
     do j=1,np
        ...
        if(isink>0.and.tp(ind_part(j)).eq.0d0)then
           ...
           enddo
           c_cloud(j)=sqrt(gamma*(gamma-1d0)*c_cloud(j)/m_cloud(j))
        endif
     enddo
  endif
     ...
     do j=1,np
        ...
        if(isink>0.and.tp(ind_part(j)).eq.0d0)then
           ...
           if(drag)then
              ...
              if(mach.le.0.95d0)factor=factor/mach**2*(0.5d0*log((1d0+mach)/(1d0-mach))-mach)
              !if(mach.gt.0.95d0.and.mach.lt.1.01d0)factor=factor*1d0
