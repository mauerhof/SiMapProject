...
subroutine make_sink(ilevel)
  ...
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min,dxx,dyy,dzz,dr_sink,rmax_sink
                          ...
                          if(dr_AGN.le.rmax2)then
                             ...
                             if(rt_AGN)then
                                do mygroup=1,nGroups
                                   Np_inj=Eini(iAGN) * group_agnegy_frac(mygroup) / (scale_evtocode*group_egy(mygroup)) / (vol_gas(iAGN)*scale_vol) / scale_Np
                                   !write(*,'(5e9.2)')dble(mygroup),Np_inj,Eini(iAGN),group_agnegy_frac(mygroup),group_egy(mygroup)
                                   rtuold(ind_cell(i),iGroups(mygroup))=rtuold(ind_cell(i),iGroups(mygroup))+Np_inj
                                enddo
                             endif
#endif
     ...
     ! ------------------------------------------                    
        ...
        if(vol_gas(iAGN)==0d0.and.ok_blast_agn(iAGN))then
           ...
           ! ------------------------------------------
              ...
              if(rt_AGN)then
                 do mygroup=1,nGroups
                    Np_inj=Eini(iAGN) * group_agnegy_frac(mygroup) / (scale_evtocode*group_egy(mygroup)) / (vol_blast(iAGN)*scale_vol) / scale_Np
                    rtuold(ind_cell(i),iGroups(mygroup))=rtuold(ind_cell(i),iGroups(mygroup))+Np_inj
                 enddo
              endif
#endif
