...
module rt_cooling_module
  ...
  public rt_set_model, rt_solve_cooling, update_UVrates, cmp_chem_eq     &
         ...
         , group_egy_erg, cs_kn, rt_prBoost, rt_AGN, group_agnegy_frac     !agnrt
  ...
  real(dp)::rt_kIR=2.28d0              ! IR vs dust opacity                 !RTpress
  real(dp)::rt_kOpt=342d0              ! Optical vs dust opacity            !RTpress
  logical::rt_AGN=.false.              ! Activate radiation from BHs?       !agnrt
  real(dp)::rt_prBoost=1d0             ! Boost on radiative pressure        !agnrt  
  ...
  real(dp),parameter::cs_kn=6.6d-25 ! [cm-2]                                !agnrt
...
!-------------------------------------------------------------------------
  ...
  if(rt) then 
     ...
     do i=1,nGroups                                                         !agnrt
        ...
        if(hRate_compton(i) .lt. 0) then   ! Compton cooling -> add photons !agnrt
        ...
        ! Scattering: Subtract phAbs bc it is included in photon flux abs.  !agnrt
     ...
     if(rt_isIR .and. rt_isOpt) then                                        !RTpress
        dU(iNpU(iGroupIR)) = dU(iNpU(iGroupIR))                          &  !RTpress
             + dU(iNpU(iGroupOpt)) * rho*Zsolar*rt_kOpt*rt_c_cgs * dt    &  !RTpress
     ...
     fracMax=dUU  ! To check if timestep size can be increased              !RTpress
     dUU=MAXVAL(                                                         &  !RTpress
     ...
     fracMax=MAX(fracMax,DUU)                                               !RTpress
  endif
  ...
  dU=dU-U
  !dUU=MAXVAL(ABS((dU)/(U+U_MIN))/U_FRAC)                                   !RTpress
  !if(dUU .lt. 0.5) then
