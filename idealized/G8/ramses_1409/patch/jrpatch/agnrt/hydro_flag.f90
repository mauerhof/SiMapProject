...
subroutine jeans_length_refine(ind_cell,ok,ncell,ilevel)
  ...
  use hydro_commons
  use poisson_commons
  use cooling_module, ONLY: twopi, XH=>X, rhoc, mH, kB
  implicit none
  ...
  real(dp)::nCOM,scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2,factG
  real(dp)::dx,dx_loc,dx_min,vol_min,scale,nISM,d0,mstar
  integer ::nx_loc
  ...
  ! From H/cc to code units
  if(cosmo)then
     nCOM = del_Jeans*omega_b*rhoc/aexp**3*XH/mH/scale_nH
  else
     nCOM= del_Jeans/scale_nH
  endif
  do i=1,ncell
     indi = ind_cell(i)
     ! the thermal energy
     ...
     ok_jeans=.true.
     if (cosmo) then
        if (dens.lt.nCOM) ok_jeans=.false.
     endif
     if (ok_jeans) then
  ...
  if(sink .or. star)then
     ...
     if(d0*(dx_loc/2.0)**ndim.lt.mstar/2d0)ok(1:ncell)=.false.
  endif
