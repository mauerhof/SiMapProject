...
!-------------------------------------------------------------------------
  ...
  nvar_count = iIons+2+ (1+ndim)*nPacs                 ! # vars (hydro+rt)
  !if(myid==1) print*,'indexes=',nvar_count,nvar,iIons,nhvar,nrtvar
  if(nvar_count .ne. nvar) then 
     if(myid==1) then 
        write(*,*) 'rt_init(): Something wrong with NVAR.'
        write(*,*) 'Should have NVAR=2+ndim+metal+dcool+aton+3+4*nPacs.'
        write(*,*) 'STOPPING!'
  ...
  if(rt_freeflow .and. myid==1) write(*,*) &
       'rt_freeflow=.true.: No interactions between photons and gas!'

  if(rt_pp) then  ! doing post-processing...then initialize all 
                  ! grid-to-particle pointers and cpu boundaries 
                  ! once and for all.     
     output_iState = .true.
     ! update boundaries
     ...
     if(pic) then 
        ! connect all particles to highest level grids containing them
  ...
  if(rt_star .and. nstar_tot .gt. 0) rt = .true.
...
101 continue                                   ! No harm if no rt namelist
  ...
  case(2)
     rt_coupling_pp=.true.
     if(myid==1 .and. (rt_star .or. rt .and. .not. rt_pp)) write(*,*)    &
  ...
  call init_UV_background ! Init UV background - homogeneous or propagated
  if(rt_star) call init_SED_table! Init stellar energy distribution props.
...
!------------------------------------------------------------------------
  ...
  ! Use the ionization energies for HI, HeI, HeII as default package 
  ! intervals
...
! uu <=  ncells*nvars: cell variables
! dx =>  real cell width
...
#endif
              ! not a 'density' for passive scalars
              if(r .gt. 0.0) then 
                 uu(i,iIons)=rt_xion_region(k)
              endif
           end do
...
!------------------------------------------------------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do ivar=1,nrtvar
           do i=1,ngrid
              uu(i,ivar)=unew(ind_cell(i),nhvar+ivar)
           end do
        end do
        ...
        do ivar=1,nrtvar
           do i=1,ngrid
              unew(ind_cell(i),nhvar+ivar)=uu(i,ivar)
           end do
        end do
     end do
     ! End loop over cells
...
!------------------------------------------------------------------------
  ...
  do k=1,rt_nsource
     ...
     pac_ind = ipac(rt_src_pac(k))-nhvar
     ...
     if(rt_source_type(k) .eq. 'point')then
        ! Volume elements
        vol=dble(dx_cgs)**ndim
        ! Compute CIC weights relative to region center
        do i=1,nn
           xn=1.0; yn=1.0; zn=1.0
           !xn=max(1.0-abs(x(i,1)-rt_src_x_center(k)+dx/4.)/dx, 0.0_dp) ! Buffer injection
           !xn=max(1.0-abs(x(i,1)-rt_src_x_center(k))/dx, 0.0_dp)
           xn=max(1.0-abs(x(i,1)-rt_src_x_center(k))/dx, 0.0)
#if NDIM>1
           !yn=max(1.0-abs(x(i,2)-rt_src_y_center(k))/dx, 0.0_dp)
           yn=max(1.0-abs(x(i,2)-rt_src_y_center(k))/dx, 0.0)
#endif
#if NDIM>2
           !zn=max(1.0-abs(x(i,3)-rt_src_z_center(k))/dx, 0.0_dp)
           zn=max(1.0-abs(x(i,3)-rt_src_z_center(k))/dx, 0.0)
#endif
           r=xn*yn*zn
           if(r .gt. 0.) then
              ! If cell lies within CIC cloud, inject value.
     ...
     if(rt_source_type(k) .eq. 'shell')then
        ...
        ! along with inner and outer radius (rt_src_length_x,y).
        ! Compute CIC weights relative to region center
        do i=1,nn
