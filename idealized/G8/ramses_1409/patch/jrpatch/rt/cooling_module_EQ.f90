...
module cooling_module
  use amr_parameters
  use rt_parameters !joki--
  implicit none
  logical :: verbose_cooling=.false.

  real(kind=8),dimension(nIons, 2)::UVrates !joki--
  ...
  !real(kind=8),parameter ::X       = 1.
  !real(kind=8),parameter ::Y       = 0. 
...
!=======================================================================
  ...
  real(kind=8)::metal,cool,heat,w1T,wT,w11,w12,w21,w22,err,yy,yy2,yy3
...
!!$        endif
        ...
        if(time(ind(i))<time_max(ind(i)) .and. T2(ind(i)) .gt. 100.)then
           n_active=n_active+1
           ind(n_active)=ind(i)
        end if
  ...
  if (tau_negative) then
     ...
     print *,'cool,cool_prime=',cool,cool_prime
     print *,'heat,heat_prime=',heat,heat_prime
     print *,'metal,metal_prime=',metal,metal_prime
     STOP
  endif
...
!=======================================================================
  ...
  !t_rad_spec=0. ; h_rad_spec=0. !joki-- NO HEATING OR RADIATIVE IONIZATION
...
!=======================================================================
  ...
  do while (err_mu > 1.d-8 .and. niter <= 501) !DEBUG joki
...
!=======================================================================
  ...
  if(ispec==HI  ) cool_bre = 1.42d-27 * 1.5 * sqrt(T) !joki
  if(ispec==HEI  ) cool_bre = 1.42d-27 * 1.5 * sqrt(T) !joki
  if(ispec==HEII  ) cool_bre = 1.42d-27 * 1.5 * sqrt(T) *4 !joki
...
!=======================================================================
  ...
  T5=1.d-5*T
  if(ispec==HI  ) cool_exc =  7.5d-19 * exp(-118348./T) / (1.+sqrt(T5)) !joki--
...
!=======================================================================
  ...
  real(kind=8)   ::T3, T6, lambda, f
  T3 = 1.d-03*T
  T6 = 1.d-06*T
  if(ispec==HI  )then !joki
     lambda   = 315614./T
     f        = 1.d0+(lambda/2.25)**0.376
     cool_rec = 3.435d-30 * lambda**1.97 / f**3.72 * T
  endif
  if(ispec==HEI  )then !joki
     lambda   = 570670./T
     cool_rec = 1.26d-14 * lambda**0.75 * kb * T
  endif
  if(ispec==HEII  )then !joki
     lambda   = 1263030./T
     f        = 1.d0+(lambda/2.25)**0.376
     cool_rec = 27.48d-30 * lambda**1.97 / f**3.72 * T
  endif
...
!=======================================================================
  ...
  if(ispec==HI )then
     lambda = 315614./T
     taux_rec =   2.753d-14 * lambda**1.5 / ( (1.d0+(lambda/2.74)**0.407)**2.242 )
  endif
  if(ispec==HEI )then
     lambda = 570670./T
     taux_rec = 1.26d-14 * lambda**0.75
  endif
  if(ispec==HEII )then
     lambda = 1263030./T
     taux_rec = 5.506d-14 * lambda**1.5                          &
                     / ( ( 1.d0+(lambda/2.74)**0.407 )**2.242 )
  endif
  !end joki--aton
  !if(ispec==HI  )taux_rec = dumfac_rec*8.40e-11/SQRT(T)/T3**(0.2)/(1.+T6**0.7) !joki--
...
!=======================================================================
  ...
  T5 = 1.d-05*T
  !if(ispec==HI  ) then
  !   cool_ion = (1.27d-21 * sqrt(T))/ ( 1.+sqrt(T5)) * exp(-157809.1/T) 
  !endif
!  dumfac_ion=1.
...
!=======================================================================
  implicit none
  real(kind=8) ::T,aexp,cool_com, Ta
  Ta     = 2.727/aexp
  cool_com     = 1.017d-37 * Ta**4 * (T-Ta) !joki-aton
  !if(cool_com .lt. 0.) cool_com=0.
...
!=======================================================================
  ...
  taux_rad_theuns=0. !joki--
  taux_rad_theuns=J0*1.63029d-18 !joki-- 
  if(ispec==HI)   taux_rad_theuns= &
       J0*( .4472*3.01d-18 + .4944*5.69d-19 + .0584*7.89d-20)
  if(ispec==HEI)  taux_rad_theuns= &
       J0*( .4944*4.48d-18 + .0584*1.20d-18)
  if(ispec==HEII) taux_rad_theuns= &
       J0*( .0584*1.06d-18)

  !J0 is interpreted as flux here (cm-2 s-1) and the constant is average 
  !cross-section for E5 K blackbody
...
!=======================================================================
  ...
  heat_rad_theuns=0. !joki--
  if(ispec==HI)   heat_rad_theuns=J0*ev*                                 &
                     ( 0.4472*(2.78d-18*18.85 - 3.01d-18*13.6d0)         &  
                      +0.4944*(5.04d-19*35.08 - 5.69d-19*13.6d0)         &
                      +0.0584*(7.46d-20*65.67 - 7.89d-20*13.6d0))
  if(ispec==HEI)  heat_rad_theuns=J0*ev*                                 &
                     ( 0.4944*(4.13d-18*35.08 - 4.48d-18*24.59d0)        &
                      +0.0584*(1.14d-18*65.67 - 1.20d-18*24.59d0))
  if(ispec==HEII) heat_rad_theuns=J0*ev*                                 &
                     ( 0.0584*(1.00d-18*65.67 - 1.06d-18*54.42d0))
