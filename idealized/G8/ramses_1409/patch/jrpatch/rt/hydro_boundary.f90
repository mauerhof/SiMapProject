!RT--Only change is reflective boundary, where photon flux
!    should be mirrored.
!################################################################
!################################################################
!################################################################
...
subroutine make_boundary_hydro(ilevel)
  ...
  integer::rtType !RT-- Type of rt variable =0,1,2,3 !-----------------!RT
        ...
        do ind=1,twotondim
           ...
           if((boundary_type(ibound)/10).ne.2)then
              ...
              do ivar=1,nvar
                 ...
                 ! ----------------------------------------------------!RT
                 ! also need to switch photon flux in RT...            !RT
                 ! rtVar= ivar-nhvar-1 = 1,2,3,4,..                    !RT
                 !                     = Np, Fx, Fy, Fz, Np, Fx,..     !RT
                 if(ivar .gt. nhvar+1) then                            !RT
                    rtType = mod(ivar-nhvar-1, ndim+1)                 !RT
                    if(rtType .ne. 0) switch=gs(rtType)                !RT
                                      !0=Np, 1=Fx, 2=Fy, 3=Fz          !RT
                 endif                                                 !RT
                 ! ----------------------------------------------------!RT
                 do i=1,ngrid
