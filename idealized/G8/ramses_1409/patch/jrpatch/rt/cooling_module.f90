module cooling_module
  ...
  public set_table, set_model, output_cool, solve_cooling, cmp_chem_eq   &
...
SUBROUTINE set_table(a_exp)
! Nothing to do here at the moment, but eventually we need to add a UV 
! model
!-------------------------------------------------------------------------
  use UV_module
  real(kind=8) :: a_exp
  real(dp),save::a_now, a_last=-1.
!-------------------------------------------------------------------------
  a_now=a_exp                                           ! Conversion to dp
  if(a_now .eq. a_last) RETURN
  a_last = a_now
  if (rt_UV_hom) call update_UVrates(a_now)
  if (rt_isDiffuseUVsrc) call update_UVsrc(a_now)
...
! Used for outputting the cooling-tables, so here there is nothing to do.
!------------------------------------------------------------------------
  implicit none
...
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
SUBROUTINE solve_cooling(U, dNpdt, dFpdt, nH,   c_switch, Zsolar         &
                          , dt,    a_exp, nCell)
...
!-------------------------------------------------------------------------
  ...
  if(rt) then 
     ...
     do i=1,nPacs           ! ------------------- Do the update of N and F
        ...
        if(q .gt. 1.d0) then           ! Normalize flux if it is too large
           if(.not. rt_smooth)                                           &
                print *,'Normalizing flux in solve_cooling(), q=',q
           !print*,'q=',q
  ...
  if(c_switch .and. cooling) then 
     ...
     if(metal) call cmp_metals(U(1),nH,mu,metal_tot,metal_prime,a_exp)
...
!-------------------------------------------------------------------------
  ...
  do while (aexp < aend)
     ...
     nH(1)  = nH_com/aexp**3
     U(1,1) = U(1,1)/aexp**2
     call solve_cooling(U,dNpdt,dFpdt,nH,c_switch,Zsolar,dt_cool,aexp,1)
