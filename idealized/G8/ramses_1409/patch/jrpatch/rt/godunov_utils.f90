!RT-- Only change here is refinement according to gradients in photon 
!     density or ionization fraction
!-------------------------------------------------------------------------
!###########################################################
!###########################################################
!###########################################################
     ...
     do i=1,nPacs                                                      !RT
        ...
        do k=1,nn                                                      !RT
           ...
           ok(k) = ok(k) .or. error > rt_err_grad_n                    !RT
                                                                       ...
                                                                       !RT
  ! Ionization state (only Hydrogen)                                   !RT
  if(rt_err_grad_xHII >= 0. .and. aexp .gt. rt_refine_aexp) then       !RT
     do k=1,nn                                                         !RT
        dg=min(1.d0,max(0.d0,ug(k,iIons)))                             !RT
        dm=min(1.d0,max(0.d0,um(k,iIons)))                             !RT
        dd=min(1.d0,max(0.d0,ud(k,iIons)))                             !RT
        error=2.0d0*MAX( &                                             !RT
             & ABS((dd-dm)/(dd+dm+rt_floor_xHII)) , &                  !RT
             & ABS((dm-dg)/(dm+dg+rt_floor_xHII)) )                    !RT
        ok(k) = ok(k) .or. error > rt_err_grad_xHII                    !RT
     end do                                                            !RT
  end if                                                               !RT

  ! Neutral state (only Hydrogen)                                      !RT
  if(rt_err_grad_xHI  >= 0. .and. aexp .gt. rt_refine_aexp) then       !RT
     do k=1,nn                                                         !RT
        dg=min(1.d0,max(0.d0,1.d0 - ug(k,iIons)))                      !RT
        dm=min(1.d0,max(0.d0,1.d0 - um(k,iIons)))                      !RT
        dd=min(1.d0,max(0.d0,1.d0 - ud(k,iIons)))                      !RT
        error=2.0d0*MAX( &                                             !RT
             & ABS((dd-dm)/(dd+dm+rt_floor_xHI)) , &                   !RT
             & ABS((dm-dg)/(dm+dg+rt_floor_xHI)) )                     !RT
        ok(k) = ok(k) .or. error > rt_err_grad_xHI                     !RT
     end do                                                            !RT
  end if                                                               !RT
...
subroutine riemann_approx(qleft,qright,qgdnv,fgdnv,ngrid)
  ...
  ! dummy arguments
  integer::ngrid
  real(dp),dimension(1:nvector,1:nhvar)::qleft,qright,qgdnv,fgdnv !----!RT
...
subroutine riemann_llf(qleft,qright,qgdnv,fgdnv,ngrid)
  ...
  ! local arrays
  real(dp),dimension(1:nvector,1:nhvar),save::fleft,fright,uleft,uright!RT
