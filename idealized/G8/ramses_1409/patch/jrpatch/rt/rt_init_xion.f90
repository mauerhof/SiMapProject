...
!-------------------------------------------------------------------------
  use amr_commons
  use hydro_commons
  use rt_parameters,only:nhvar,nIons
...
!-------------------------------------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do i=1,nleaf
        ...
        uold(ind_leaf(i),nhvar-nIons+1) = x*uold(ind_leaf(i),1)
        if(Y .gt. 0.d0 .and. nIons .ge. 3) then
           x = nSpec(5)/(nSpec(4)+nSpec(5)+nSpec(6))      !  HeII fraction
           uold(ind_leaf(i),nhvar-nIons+2) = x*uold(ind_leaf(i),1)         
           x = nSpec(6)/(nSpec(4)+nSpec(5)+nSpec(6))      ! HeIII fraction
           uold(ind_leaf(i),nhvar-nIons+3) = x*uold(ind_leaf(i),1)         
        endif
      end do
...
!-------------------------------------------------------------------------
  use cooling_module,only:signc
  use amr_commons
  use hydro_commons
  use rt_parameters
  use cooling_module,only:Y,UVrates
  implicit none
