!RT-- Only change is use of nhvars rather than nvars
!=========================================================================
! ---------------------------------------------------------------
...
subroutine unsplit(uin,gravin,flux,tmp,dx,dy,dz,dt,ngrid)
  ...
  ! Input states
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar)::uin !-!RT
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:ndim)::gravin 

  ! Output fluxes
  real(dp),dimension(1:nvector,if1:if2,jf1:jf2,kf1:kf2,1:nhvar,1:ndim)&!RT
       ::flux !--------------------------------------------------------!RT
  real(dp),dimension(1:nvector,if1:if2,jf1:jf2,kf1:kf2,1:2   ,1:ndim)::tmp 

  ! Primitive variables
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar) & !----!RT
       ,save::qin !----------------------------------------------------!RT
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2       ),save::cin

  ! Slopes
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar,1:ndim)&!RT
       ,save::dq !-----------------------------------------------------!RT
...
!###########################################################
!Joki:                                                                 !RT
!The trace routines are for finding values at cell sides               !RT
!in the piecewise linear approximation. See p. 425 in Toro.            !RT
subroutine trace1d(q,dq,qm,qp,dx,dt,ngrid)
  ...
  !BEGIN RT------------------------------------------------------------!RT
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar)::q 
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar,1:ndim)::dq 
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar,1:ndim)::qm 
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar,1:ndim)::qp 
  !END RT--------------------------------------------------------------!RT
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              r   =  q(l,i,j,k,ir) ! density
              u   =  q(l,i,j,k,iu) ! momentum
              p   =  q(l,i,j,k,ip) ! pressure
...
subroutine cmpflxm(qm,im1,im2,jm1,jm2,km1,km2, &
  ...
  integer ::ilo,ihi,jlo,jhi,klo,khi
  !BEGIN RT------------------------------------------------------------!RT
  real(dp),dimension(1:nvector,im1:im2,jm1:jm2,km1:km2,1:nhvar,1:ndim)::qm
  real(dp),dimension(1:nvector,ip1:ip2,jp1:jp2,kp1:kp2,1:nhvar,1:ndim)::qp 
  real(dp),dimension(1:nvector,ip1:ip2,jp1:jp2,kp1:kp2,1:nhvar)::flx
  !BEGIN RT------------------------------------------------------------!RT
  ...
  real(dp)::entho
  real(dp),dimension(1:nvector,1:nhvar),save::qleft,qright,qgdnv,fgdnv !RT
...
subroutine ctoprim(uin,q,c,gravin,dt,ngrid)
  ...
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar)::uin   !RT
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:ndim)::gravin
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nhvar)::q     !RT
