!RT patch: Replaced one instance of nvar with nhvar (which probably 
!          isn't even neccessary in which case this file can be thrown
!          from the RT patch).
!*************************************************************************
!################################################################
!################################################################
...
subroutine init_flow_fine(ilevel)
  ...
  use hydro_commons
  use cooling_module
  use rt_parameters, ONLY: nhvar !-------------------------------------!RT
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  ...
  if(ok_file)then ! initalization from grafics type files
     ...
     do ivar=1,nvar
        ...
        else ! For Iliev test 5 initialization:                        !RT
           ! Rescale initial conditions to code units                  !RT
           if(ivar==1)init_array=init_array/scale_nH                   !RT
           if(ivar==2)init_array=init_array/scale_v                    !RT
           if(ivar==3)init_array=init_array/scale_v                    !RT
           if(ivar==4)init_array=init_array/scale_v                    !RT
           if(ivar==5)init_array=init_array                      &     !RT
                                *(scale_t/scale_l)**2/scale_d          !RT
        endif
...
subroutine region_condinit(x,q,dx,nn)
  ...
  do ivar=ndim+3,nvar   ! passive scalars and RT vars set to 0
     q(1:nn,ivar)=0.0d0
  end do
