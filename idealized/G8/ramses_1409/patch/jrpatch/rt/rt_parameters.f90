module rt_parameters
...
#else
  integer,parameter::nPacs=0
#endif
  integer,parameter::nRTvar=nPacs*(1+ndim)    ! # of RT variables (photon density and flux)
  integer,parameter::nHvar=nvar-nRTvar                              ! # of hydro variables
  ...
  logical::rt_is_init_xion=.false.     ! Initialize ionization fractions from T profile? !
  ...
  integer,dimension(nPacs)::iPac=nhvar+1                         !   Start indices of pacs
  ...
  real(dp),dimension(1:MAXREGION)   ::rt_xion_region=0.  ! xHII state (square regions only)
  ...
  logical::doDump = .false.            ! Force an output dump when next possible         !
