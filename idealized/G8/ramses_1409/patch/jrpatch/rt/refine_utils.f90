!RT patch: An optional enforcement of equilbrium ionization states when
!          uploading.
!=========================================================================
...
subroutine kill_grid(ind_cell,ilevel,nn,ibound,boundary_region)
  ...
  real(dp),dimension(nIons)::xion !------------------------------------!RT
  
  ! -------------------------------------------------------------------!RT
  if(upload_equilibrium_x) then                                        !RT
     ! Enforce equilibrium on ionization states when merging, to       !RT
     ! prevent unnatural values (e.g when merging hot and cold cells). !RT
     do i=1,nn                                                         !RT
        call calc_equilibrium_xion(uold(ind_cell(i),1:nvar), xion)     !RT
        uold(ind_cell(i),iIons:iIons+nIons-1)=xion*uold(ind_cell(i),1) !RT
     enddo                                                             !RT
  endif                                                                !RT
  ! -------------------------------------------------------------------!RT
