  ...
  do ind=1,twotondim
     ...
     do ip=1,nPacs
        do i=1,active(ilevel)%ngrid
           ...
           uold(icell,iPac(ip)) = max(uold(icell,iPac(ip)),smallNp)
           Npc=uold(icell,iPac(ip))*rt_c
           fred = sqrt(sum( &      ! Reduced flux, should always be .le. 1
                     (uold(icell,iPac(ip)+1:iPac(ip)+ndim))**2))/Npc
           if(fred .gt. 1.d0) then !      Too big so normalize flux to one
              uold(icell,iPac(ip)+1:iPac(ip)+ndim) =             &
              uold(icell,iPac(ip)+1:iPac(ip)+ndim)/fred
           endif
        end do
     end do
     ! End make a photon conservation fix
...
!------------------------------------------------------------------------
  ...
  do i1=i1min,i1max
     ...
     do j=0,twondim
        do ivar=1,nrtvar
           do i=1,nbuffer
              u1(i,j,ivar)=uold(ibuffer_father(i,j),ivar+nhvar)
           end do
        end do
     end do
     call interpol_rtvars(u1,u2,nbuffer)
  ...
  !----------------------------------------------------------------------
  do i=1,nPacs
     call cmp_rt_faces(uloc, flux, dx, dx, dx, dt, iPac(i)-nhvar, ncache)
  end do
  ...
  do idim=1,ndim
     ...
     do ivar=1,nrtvar
        ...
        do i3=i3min,i3max-i0 ! 1 to 1 if dim=1, 1 to 2 otherwise
           do i=1,nb_noneigh
              unew(ind_buffer(i),ivar+nhvar)=unew(ind_buffer(i),ivar+nhvar) &
