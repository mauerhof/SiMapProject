...
!-------------------------------------------------------------------------
  ...
  integer::nv=2+2*nIons      ! # vars in SED table: L,Lacc,nions*(cse,egy)
!-------------------------------------------------------------------------
  if(myid==1) &
...
! simulation, weighted by the luminosity of the particles. If there are no
...
!-------------------------------------------------------------------------
  ...
  do i=1,npartmax
     ...
     nstars = nstars + 1
     mass = mp(i)
     call getAgeGyr(tpp(i), age)                        !     age = [Gyrs]
     if(metal) then
...
!-------------------------------------------------------------------------
  ...
  do j = 1, np
     ...
     call getAgeGyr(tpp(ind_part(j)), age)         !  End-of-dt age [Gyrs]
...
!-------------------------------------------------------------------------
  ...
  ! Non-propagated UV background (not propagated)-------------------------
  if(rt_UV_hom) then
...
!         1=photon flux [#/cm2/s], 2+2*i=pac_csn[cm-2], 3+2*i=pac_egy [ev]
!-------------------------------------------------------------------------
  real(dp), intent(in):: z
...
!-------------------------------------------------------------------------
  ...
  if(rt .and. redshift .le. UV_maxz .and. myid==1) then
