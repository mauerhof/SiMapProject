...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if(ilevel .eq. levelmin) call output_SN_stats ! Once per coarse step    !DSfeedb
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
subroutine feedbk(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  !real(dp)::dx,dx_loc,scale,vol_loc,birth_time,current_time               !joki
  real(dp)::dx,dx_loc,scale,birth_time,current_time                        !joki
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  logical::error
  ! Grid based arrays
  ...
  real(dp),dimension(1:nvector),save::vol_loc                              !joki
  real(dp),dimension(1:nvector,1:ndim),save::x
  integer ,dimension(1:nvector,1:ndim),save::id,igd,icd
  integer ,dimension(1:nvector),save::igrid,icell,indp,kg
  real(dp),dimension(1:3)::skip_loc

  real(dp)::prob,de_min,mcell !joki
  ...
  dx_loc=dx*scale
  !vol_loc=dx_loc**ndim                                                    !joki
  vol_loc(1:nvector)=dx_loc**ndim                                          !joki
  ...
  if(use_proper_time)then
     t0=SN_delay_Myr*1d6*(365.*24.*3600.)/(scale_t/aexp**2)    !Cev patch
     current_time=texp
  else
     t0=SN_delay_Myr*1d6*(365.*24.*3600.)/scale_t              !Cev patch
     current_time=t
  endif
  ...
  do j=1,np
     if(ok(j))then
        ...
        indp(j) = nbors_father_cells(ind_grid_part(j),kg(j)) !joki
        vol_loc(j)=vol_loc(j)*2**ndim ! ilevel-1 cell volume !joki
     end if
  end do
     ...
     do j=1,np
        !if(ok(j))then                                      !joki
           ...
           if(birth_time.lt.(current_time-t0).and.birth_time.ge.(current_time-t0-dteff(j)))then
              ...
              mloss(j)=mloss(j)+mejecta/vol_loc(j)          !joki
              ! Thermal energy
              ethermal(j)=ethermal(j)+mejecta*ESN/vol_loc(j) !joki
              ! Metallicity
              if(metal)then
                 zloss=yield+(1d0-yield)*zp(ind_part(j))
                 mzloss(j)=mzloss(j)+mejecta*zloss/vol_loc(j) !joki
              endif
              ...
              if(SN_dT2_min .gt. 0d0) then
                 ...
                 !mcell = uold(indp(j),1) * vol_loc    !joki--
                 mcell = unew(indp(j),1) * vol_loc(j)    !joki++
                 prob = mejecta*ESN / de_min / (mcell+mejecta) ! SN probability
                 !if(prob.lt.1) then ! Stochastic SNe to fill in required deltaT
                 if(prob.le.1d0) then ! Stochastic SNe to fill in required deltaT
                    n_pr = n_pr + 1                                       ! Statistics
                    pr_tot = pr_tot + prob                                   ! Statistics
                    call ranf(localseed,RandNum)
                    !if(RandNum.le.prob) then
                    if(RandNum<prob) then
                       ethermal(j) = de_min * (mcell+mejecta) / vol_loc(j) !joki
                       !print*,'HERE: ',ethermal(j)/unew(indp(j),1)*scale_T2
              ...
              if(ethermal(j) .gt. 0d0) then 
                 !st_n_SN(ind_part(j)) = uold(indp(j),1)       !-----------!SD !joki--
                 st_n_SN(ind_part(j)) = unew(indp(j),1)       !-----------!SD
                 st_e_SN(ind_part(j)) = ethermal(j) * vol_loc(j) * scale_d  &!SD !joki
                      * scale_l**3 * scale_v**2 / 1d51        !-----------!SD
              endif
              E_SN_got = E_SN_got + ethermal(j) * vol_loc(j)                 ! Statistics !joki
              ! Good to write out:
              !   ratio of p<1 to p>1
              !   average of p<1
              !   Total wanted ejected mass and total ejected mass
              !   Total wanted injected energy and total injected energy
              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           endif
        end if                                               !joki
     end do
  ...
  do j=1,np
     !if(ok(j))then                                                        !joki--
        ...
        if(metal)then
           !tauIR=tau_factor*max(uold(indp(j),imetal),smallr)              !joki--
           tauIR=tau_factor*max(unew(indp(j),imetal),smallr)               !joki++
        else
           !tauIR=tau_factor*max(uold(indp(j),1),smallr)                   !joki--
           tauIR=tau_factor*max(unew(indp(j),1),smallr)                    !joki++
        endif
        ...
        !uold(indp(j),1)=uold(indp(j),1)+mloss(j)                          !joki--
        !uold(indp(j),2)=uold(indp(j),2)+mloss(j)*vp(ind_part(j),1)        !joki--
        !uold(indp(j),3)=uold(indp(j),3)+mloss(j)*vp(ind_part(j),2)        !joki--
        !uold(indp(j),4)=uold(indp(j),4)+mloss(j)*vp(ind_part(j),3)        !joki--
        !uold(indp(j),5)=uold(indp(j),5)+mloss(j)*ekinetic(j)+ &           !joki--
        !     & ethermal(j)*(1d0+RAD_BOOST)                                !joki--

        unew(indp(j),1)=unew(indp(j),1)+mloss(j)                           !joki++
        unew(indp(j),2)=unew(indp(j),2)+mloss(j)*vp(ind_part(j),1)         !joki++
        unew(indp(j),3)=unew(indp(j),3)+mloss(j)*vp(ind_part(j),2)         !joki++
        unew(indp(j),4)=unew(indp(j),4)+mloss(j)*vp(ind_part(j),3)         !joki++
        unew(indp(j),5)=unew(indp(j),5)+mloss(j)*ekinetic(j)+ &            !joki++
             & ethermal(j)*(1d0+RAD_BOOST)                                 !joki++
     !endif                                                                !joki--
  end do
  ...
  if(metal)then
     do j=1,np
        !if(ok(j))then                                                     !joki--
           !uold(indp(j),imetal)=uold(indp(j),imetal)+mzloss(j)            !joki--
           unew(indp(j),imetal)=unew(indp(j),imetal)+mzloss(j)             !joki++
        !endif                                                             !joki--
     end do
  endif
  ...
  if(delayed_cooling)then
     do j=1,np
        !if(ok(j))then                                                     !joki--
           !uold(indp(j),idelay)=uold(indp(j),idelay)+mloss(j)             !joki--
           unew(indp(j),idelay)=unew(indp(j),idelay)+mloss(j)              !joki++
        !endif                                                             !joki--
     end do
  endif
