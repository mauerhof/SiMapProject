! Additional stuff by joki for outputting thermal SNe statistics
module SN_stats
...
subroutine thermal_feedback(ilevel,icount)
  ...
  real(dp)::current_time                                                  !joki

  if(f_w .ne. 0d0)  return                                                ! joki
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if( idp(ipart).lt.0 .and. tp(ipart).ne.0 .and. &            ! joki
                   tp(ipart).lt.(current_time-t0) )then 
                 npart2=npart2+1
              endif
...
!        birth_time=tp(ind_part(j))
        ...
        if( idp(ind_part(j)).lt.0 .and. tp(ind_part(j)).ne.0d0 .and. &
             & tp(ind_part(j)).lt.(current_time-t0) )then
           ! Stellar mass loss
           ...
           if(SN_dT2_min .gt. 0d0) then
              ...
              if(prob.le.1d0) then ! Stochastic SNe to fill in required deltaT
                 ...
                 if(RandNum<prob) then
                    ethermal(j) = de_min * (mcell+mejecta) / vol_loc
                    !print*,'HERE: ',ethermal(j)/uold(indp(j),1)*scale_T2
           ...
           E_SN_got = E_SN_got + ethermal(j) * vol_loc                 ! Statistics
           ! Good to write out:
           !   ratio of p<1 to p>1
           !   average of p<1
...
113 format('SN_stats ',1pe12.5,1pe12.5,1pe12.5,1pe12.5,i7,i7,i7)
