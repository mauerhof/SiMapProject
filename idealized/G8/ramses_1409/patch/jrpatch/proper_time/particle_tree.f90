! PT (proper time) patch:
! Added tpp on nov 18 2009. Here, tpp is added to communication buffers
! for moving particles between cpus.
!-------------------------------------------------------------------------
!################################################################
!################################################################
!################################################################
...
subroutine virtual_tree_fine(ilevel)
  ...
  do icpu=1,ncpu
     ...
     if(ncache>0)then
        ...
        if(star.or.sink)then
           if(metal)then
              allocate(reception(icpu,ilevel)%up(1:ncache,1:twondim+4))!PT
           else
              allocate(reception(icpu,ilevel)%up(1:ncache,1:twondim+3))!PT
           endif
        else
  ...
  do icpu=1,ncpu
     ...
     if(ncache>0)then
        ...
        if(star.or.sink)then
           if(metal)then
              allocate(emission(icpu,ilevel)%up(1:ncache,1:twondim+4)) !PT
           else
              allocate(emission(icpu,ilevel)%up(1:ncache,1:twondim+3)) !PT
           endif
        else
  ...
  do icpu=1,ncpu
     ...
     if(ncache>0)then
        ...
        if(star.or.sink)then
           if(metal)then
              buf_count=ncache*(twondim+4) !---------------------------!PT
           else
              buf_count=ncache*(twondim+3) !---------------------------!PT
           endif
        else
  ...
  if(star.or.sink)then
     ...
     if(metal)then
        do i=1,np
           ...
           reception(icpu,ilevel)%up(ind_com(i),twondim+4)= & !--------!PT
                tpp(ind_part(i))                              !--------!PT
        end do
     else
        do i=1,np
           reception(icpu,ilevel)%up(ind_com(i),twondim+3)= & !--------!PT
                tpp(ind_part(i))                              !--------!PT
        end do
     end if
  end if
  ...
  if(star.or.sink)then
     ...
     if(metal)then
        do i=1,np
           ...
           tpp(ind_part(i))= & !---------------------------------------!PT
                emission(icpu,ilevel)%up(ind_com(i),twondim+4) !-------!PT
        end do
     else
        do i=1,np
           tpp(ind_part(i))= & !---------------------------------------!PT
                emission(icpu,ilevel)%up(ind_com(i),twondim+3) !-------!PT
        end do
     end if
  end if
