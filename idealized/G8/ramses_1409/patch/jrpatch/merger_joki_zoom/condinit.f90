...
module merger_parameters!{{{
  ...
  ! For zooming in on one cylindrical region in the disk (Joki, Feb 2015):
  real(dp),dimension(3)::ref_region_center=0d0
  real(dp)::ref_region_radius=0d0, ref_region_height=0d0
...
subroutine read_merger_params! {{{
  ...
  namelist/merger_params/ IG_density_factor &
       ...
       & ,ref_region_center, ref_region_radius, ref_region_height
...
subroutine condinit(x,u,dx,nn)
  ...
  do i=1,nn
    ...
    ! Projected cell position over galactic centers axis
     ...
     ! Cylindrical refinement region in disk:
     if(ivar_refine .gt. 0) then
        q(i,ivar_refine)=0d0 ! No refinement if outside region
        xx1=x(i,:)-(ref_region_center(:)+boxlen/2.0D0)
        dh = abs(xx1(3))  ! Height from region centre 
        dr = sqrt(xx1(1)**2 + xx1(2)**2) ! xy-plane radius from region centre
        if(dh .lt. ref_region_height .and. dr .lt. ref_region_radius) then
           q(i,ivar_refine)=1d0
        endif   
     endif
     ! End cylindrical refinement region
