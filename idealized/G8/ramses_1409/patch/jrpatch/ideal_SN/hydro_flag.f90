! Ideal_SN patch: Added a call to ideal_SN_refine
! ---------------
! RT patch: Only change here is to possibly do refinement if rt_err_grad_n
!           or rt_err_grad_xion namelist parameters are set
...
subroutine hydro_flag(ilevel)
  ...
  !ideal_SN PATCH BEGIN---------------------------------------------------
  !if(    err_grad_d==-1.0.and.          &
  !     & err_grad_p==-1.0.and.          &
  !     & err_grad_u==-1.0.and.          &
  !     & jeans_refine(ilevel)==-1.0.and.&
  !     & rt_err_grad_n==-1.0 .and.      & !----------------------------!RT
  !     & rt_err_grad_xion==-1.0         & !----------------------------!RT
  !& ) &
  !    return
  !ideal_SN PATCH END------------------------------------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        !if(r_refine(ilevel)>-1.0)then !-------------------------!ideal_SN
           ...
           call ideal_SN_refine(xx,ok,ngrid,ilevel) !------------!ideal_SN
        !end if !------------------------------------------------!ideal_SN
