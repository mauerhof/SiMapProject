subroutine units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  ...
  !-----------------------------------------------------------------------
  real(dp)::v200,c,r200,h,omegam2,redshift2

  v200 = gravity_params(1) ! Virial velocity of the halo (km.s-1)
  c = gravity_params(2)    ! Concentration of the halo
  omegam2 = gravity_params(5)   ! Mass density
  redshift2 = gravity_params(6) ! redshift
  
  ! NFW scale radius
  h = 0.7
  r200 = v200*1d5/sqrt(2d0*twopi*6.67d-8/3.*200.*rhoc*(omegam2*(1d0+redshift2)**3)*h**2)

  ! scale_d converts mass density from user units into g/cc
  scale_d = rhoc*(omegam2*(1d0+redshift2)**3)*h**2*200./3.0*c*c*c/(log(1d0+c)-c/(1d0+c))
