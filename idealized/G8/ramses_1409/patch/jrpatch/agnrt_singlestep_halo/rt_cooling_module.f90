...
module rt_cooling_module
  ...
  public rt_set_model, rt_solve_cooling, update_UVrates, cmp_chem_eq     &
         ...
         , iP0, iP1, iPtot, rt_isoPress, rt_pressBoost                   & !RTpress
         , rt_isIR, rt_isOpt, rt_kIR, rt_kOpt, rt_kUV                    & !RTpress
  ...
  real(dp)::rt_kIR=1d1                 ! IR vs dust opacity                 !RTpress
...
!-------------------------------------------------------------------------
  ...
  if(myid==1 .and. rt_isopress) write(*,*) &
       '======Isotropic nonthermal RT pressure is turned ON=============='
  ! do initialization
...
!-------------------------------------------------------------------------
  ...
  ! START DEBUG-----------------------------------------------------------
  !if(loopcnt .gt.4000) then ! print out history and quit ramses
  !   lastj=1
  !   do j=1,loopcnt
  !      dU=history(i,:,j)
...
!-------------------------------------------------------------------------
  ...
  real(dp):: rho, rdAbs_IR, rdAbs_Opt, rdAbs_UV                             !RTpress
  real(dp):: grAbs                                                          !agnrt
...
!-------------------------------------------------------------------------
  ...
  neInit=ne
  rho = nH / X * mH                                                        !RTpress
  rdAbs_IR  = rho*Zsolar*rt_kIR*rt_c_cgs  ! IR-dust absorption rate [s-1]  !RTpress
  ...
  if(rt) then 
     ...
     if(rt_isIR) then !IR scattering on dust                                !RTpress
        phAbs(iGroupIR) = phAbs(iGroupIR) + 0.001 * rdAbs_IR                !RTpress
        phSc(iGroupIR)  = phSc(iGroupIR)  + 0.999 * rdAbs_IR                !RTpress
     endif                                                                  !RTpress
