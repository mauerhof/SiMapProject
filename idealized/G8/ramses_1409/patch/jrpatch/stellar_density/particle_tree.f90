! SD (stellar density) patch:
! Added  st_n_tp and st_n_sn variables (oct 2013)
! Also feedback at 5 Myr.
!################################################################
!################################################################
!################################################################
!################################################################
...
subroutine virtual_tree_fine(ilevel)
  ...
  if(star.or.sink) then
     if(metal) then
        !particle_data_width=twondim+3                                    !SD
        particle_data_width=twondim+6                                     !SD
     else
        !particle_data_width=twondim+2                                    !SD
        particle_data_width=twondim+5                                     !SD
     endif
  endif
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     current_property=current_property+1                                  !SD
     if(metal)then
        do i=1,np
           reception(icpu,ilevel)%up(ind_com(i),current_property)        &!SD
                =zp(ind_part(i))                                          !SD
        end do
        current_property=current_property+1                               !SD
     end if
     ...
     do i=1,np                                                            !SD
        reception(icpu,ilevel)%up(ind_com(i),current_property)           &!SD
             =st_n_tp(ind_part(i))                                        !SD
     end do                                                               !SD
     current_property=current_property+1                                  !SD

     do i=1,np                                                            !SD
        reception(icpu,ilevel)%up(ind_com(i),current_property)           &!SD
             =st_n_SN(ind_part(i))                                        !SD
     end do                                                               !SD
     current_property=current_property+1                                  !SD

     do i=1,np                                                            !SD
        reception(icpu,ilevel)%up(ind_com(i),current_property)           &!SD
             =st_e_SN(ind_part(i))                                        !SD
     end do                                                               !SD
     current_property=current_property+1                                  !SD
