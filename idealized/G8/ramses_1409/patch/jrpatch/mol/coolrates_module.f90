...
ELEMENTAL FUNCTION comp_Alpha_H2(T)
...
!-------------------------------------------------------------------------
  ...
  if(isH2) then
     fT3 = T/1.d3
     lterleft = 9.5d-22 * fT3**3.76  &
          * exp(-1.0d0*(0.13d0/fT3)**3) / (1.0d0+0.12d0*fT3**2.1)
     ...
     lowvright_hi=exp(-11720.0/T)*1.6d-12*1.6d-12*sqrt(T) &
                                                   *exp(-1.0*(4.0d2/T)**2)
     ...
     coolH2 = nN(ixHI)*ltetot                                            &
            / (1.0+ltetot/(nN(ixHII)*lowtot_hi+nN(1)*lowtot_h2))
     
     TT=T*1.0001
     fTT3= TT/1.d3
     lterleft = 9.5d-22*fTT3**3.76*exp(-1.0d0*(0.13d0/fTT3)**3)          &
              / (1.0d0+0.12d0*fTT3**2.1)
     ...
     lowvright_hi=exp(-11720.0/TT)*1.6d-12*1.6d-12*sqrt(TT) &
                 *exp(-1.0*(4.0d2/TT)**2)
     lowv_hi=lowvleft_hi+lowvright_hi
     ...
     dcoolH2=(nN(ixHI)*ltetot/(1.0+ltetot/(nN(ixHII)*lowtot_hi+nN(ixHI)  &
             *lowtot_h2))-coolH2) / (0.0001*T)
  endif
