MODULE ramses_info
 ...
 ! All units are cgs
  ...
  logical::mechanical_feedback
  character(LEN=80)::star_maker, star_imf!, mechanical_feedback
...
SUBROUTINE read_info_int(isnap)

! Read info files
!-------------------------------------------------------------------------
  integer,intent(in):: isnap
  character(len=1000)::str_nsnap
!-------------------------------------------------------------------------
  call title(isnap,nchar)
  str_nsnap = 'output_'//TRIM(nchar)//'/'
  call read_info(str_nsnap)

END SUBROUTINE read_info_int

!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
SUBROUTINE read_info(str_nsnap)

! Read info files
!-------------------------------------------------------------------------
  character(len=1000),intent(in)::str_nsnap
  character*1000::filename,dirname
  character(len=1000)::ifmt,dfmt,varname
  integer::iargc,ipos
  logical::ok
  integer::ilun, i, impi
  real::value
...
101 continue                                   ! No harm if no rt namelist
  ...
  if (.not. ok) then
     ...
     read(ilun,*)
     read(ilun,'(13x,E23.15)')scale ! = boxlen
  ...
  filename=TRIM(dirname)//'header_'//TRIM(nchar)//'.txt'
  inquire(file=filename, exist=ok)
  if (.not. ok) then
     write(*,*)'File '//TRIM(filename)//' not found'
     STOP
...
111 continue                        ! eof reached, didn't find the parameter
  ...
  if(cosmo)then
     ...
     if(doprints) write(*,*)'Time simu (Gyr) / Hubble time = ',time_simu_gyr, ' / '   &
                           ,(time_tot)/(h0*1d5/3.08d24)/(365.*24.*3600.*1d9)
  else
     time_simu=t*unit_t/(365.*24.*3600.*1d9)
...
!-------------------------------------------------------------------------
  ...
  integer::ind, i, iGroup, indexGroup
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  ! Surface densities, should use tot=1:
     ...
     emType='oneVar' ; emVar=1 ; defUnit=unit_nHe*scale*unit_l
  else if(varName .eq. 'SnHeI') then !------------------------------------
     emType='K' ; emVar=iIons+2 ; defUnit=unit_nHe*scale*unit_l
  else if(varName .eq. 'SnHeII') then !-----------------------------------
     emType='p' ; emVar=iIons+1 ; defUnit=unit_nHe*scale*unit_l
  else if(varName .eq. 'SnHeIII') then !----------------------------------
     emType='p' ; emVar=iIons+2 ; defUnit=unit_nHe*scale*unit_l
  else if(varName .eq. 'SnHZ') then !-------------------------------------
     emType='SnHZ' ; defUnit=unit_nH*scale*unit_l
