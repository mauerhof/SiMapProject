PROGRAM O_fractions
  ...
  !Variables for Krome
  integer,parameter		:: nBinK=krome_nPhotoBins, nsp=krome_nmols, nIon=3, nPhoto=krome_nPhotoRates
  real(kind=8)                  :: densities(nsp), binsK(nBinK+1)


  integer(kind=4)               :: ncells, nSEDgroups, nfields, nstars, ncpu_read
  real(kind=8)                  :: rates_O(nPhoto), nH_i, nHe, nO, Tgas_i, xHII, xHeII, xHeIII
  integer(kind=4),allocatable   :: cell_level(:), cpu_list(:)
  ...
  if(rank==master) then
     ...
     print*, 'number of stellar particles : ', nstars
  ...
  if(reading_method=='fromlist') then
     ...
     if(rank==master) then
        ncpu_read = get_ncpu(ramDir,timestep)
        allocate(cpu_list(1:ncpu_read))
        cpu_list(:) = (/ (i, i=1,ncpu_read) /)
        call read_leaf_cells_omp(ramDir, timestep, ncpu_read, cpu_list, ncells, nfields, cell_pos, cells, cell_level)   ; deallocate(cell_pos,cell_level)
  ...
  do icells=1,chunksize(rank+1)
     ...
     if(Tgas_i > 1d6) then
        ...
        elseif(Tgas_i > 5.5d4) then
           densities(:) = (/ max(nH_i*xHII + nHe*(xHeII + 2*xHeIII) + 2*nO,1d-18), max(nH_i*(1-xHII),1d-18), max(nHe*(1-xHeII-xHeIII),1d-18), 1d-18, max(nH_i*xHII,1d-18), max(nHe*xHeII,1d-18), 1d-18, max(nHe*xHeIII,1d-18), max(nO,1d-18), 1d-18 /)
        !Here, the gas is ~dominated by OII
        elseif(Tgas_i > 1.6d4) then
           densities(:) = (/ max(nH_i*xHII + nHe*(xHeII + 2*xHeIII) + nO,1d-18), max(nH_i*(1-xHII),1d-18), max(nHe*(1-xHeII-xHeIII),1d-18), 1d-18, max(nH_i*xHII,1d-18), max(nHe*xHeII,1d-18), max(nO,1d-18), max(nHe*xHeIII,1d-18), 1d-18, 1d-18 /)
        !Dominated by OI
        else
           densities(:) = (/ max(nH_i*xHII + nHe*(xHeII + 2*xHeIII),1d-18), max(nH_i*(1-xHII),1d-18), max(nHe*(1-xHeII-xHeIII),1d-18), max(nO,1d-18), max(nH_i*xHII,1d-18), max(nHe*xHeII,1d-18), 1d-18, max(nHe*xHeIII,1d-18), 1d-18, 1d-18 /)
        end if
        
        call krome_equilibrium(densities(:), Tgas_i)

        if(max(densities(4), densities(7), densities(9), densities(10))/nO > 1.0001) then  !cell bugs,  < 1/100000,   no real solution
           count = count + 1
           print*, 'bug'
           if(Tgas_i < 1.6d4) then
              O_cpu(:,icells) = (/ nO, 1d-18, 1d-18 /)
           elseif(Tgas_i < 5.5d4) then
              O_cpu(:,icells) = (/ 1d-18, nO, 1d-18 /)
           elseif(Tgas_i < 1.14d5) then
              O_cpu(:,icells) = (/ 1d-18, 1d-18, nO /)
           else
              O_cpu(:,icells) = (/ 1d-18, 1d-18, 1d-18 /)
           endif
        !no bug
        else
           O_cpu(:,icells) = (/ max(densities(4),1d-18), max(densities(7),1d-18), max(densities(9),1d-18) /)
        endif
