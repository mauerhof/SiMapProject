...
!			 !!TEST!!!!
!             !Hrate = Hrate + dNp(igroup) * SUM(nN(:) * PHrate(igroup,:))   !ICII
!             Hrate = Hrate + 0 * SUM(nN(:) * PHrate(igroup,:))
!             !TEST!!!!
!          end do
!       endif
...
!    endif
    ...
    if(rt_isIR) then
       if(kAbs_loc(iIR) .gt. 0d0 .and. .not. rt_T_rad) then
          ...
          !!!!!TEST!!!!
          E_rad = group_egy_erg(iIR) * dNp(iIR)  !!!ICIII!!!!
          !E_rad = group_egy_erg(iIR) * 0
          !1!!!TEST!!!!
          ...
          !!!!!TEST!!!!
          dUU = ABS(dNp(iIR)-Np(iIR,icell)) / (Np(iIR,icell)+Np_MIN)     &  !!ICIII!!!
                                            * one_over_Np_FRAC
          !dUU = ABS(0-Np(iIR,icell)) / (Np(iIR,icell)+Np_MIN)     &  !!ICIII!!!
           !                                 * one_over_Np_FRAC
          !!!!!TEST!!!!
          if(dUU .gt. 1.) then                 
             code=4 ;   RETURN                          
          ...
          TK=dT2*mu
          !!!!TEST!!!!
          call reduce_flux(dFp(:,iIR),dNp(iIR)*rt_c_cgs)            !!!ICIII
          !call reduce_flux(dFp(:,iIR),0*rt_c_cgs)
          !!!!TEST!!!!
       endif
    endif
#endif
