...
def orderCell(cell_pos, toBeOrdered):
	...
	orderVector = np.zeros_like(cell_pos[:,0])
	for i in range(3):
		orderVector = orderVector + math.sqrt(i+1)*cell_pos[:,i] 
...
def ScatterPlot(x,y,size,xlabel,ylabel,name):

	#xy_line = (0, 1)
	plt.style.use('dark_background')
	fig, ax = plt.subplots(figsize=(8,8))
	ax.scatter(x, y, s=size, marker='.', facecolor='0.5', lw=0)
	plt.xlim(0,1)
	plt.ylim(0,1)
