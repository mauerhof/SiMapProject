...
!-------------------------------------------------------------------------
  ...
  integer::locid,ncpu2
  integer::nv=3+2*(nIons+4)  ! # vars in SED table: L,Lacc,egy,nions*(csn,egy)
...
!-------------------------------------------------------------------------
  ...
  do ip = 1,nSEDgroups                                ! Loop photon groups
     ...
     do ia = locid+1,nAges,ncpu2                                ! Loop age
        ...
        do ii = 1,nIonsUsed+4                                ! Loop species
...
!-------------------------------------------------------------------------
  ...
  real(dp),save,allocatable,dimension(:,:)::csn_star, cse_star, Si_csn_star
  real(dp),save,allocatable,dimension(:)::  egy_star
  real(dp),save,allocatable,dimension(:)::  sum_L_cpu,sum_L_all
  real(dp),save,allocatable,dimension(:,:)::sum_csn_cpu,sum_csn_all, sum_Si_csn_cpu, sum_Si_csn_all
...
!-------------------------------------------------------------------------
  if(.not. allocated(L_star)) then
     ...
     allocate(Si_csn_star(nSEDgroups,4))
     ...
     allocate(sum_Si_csn_cpu(nSEDgroups,4))
     allocate(sum_csn_all(nSEDgroups,nIons)) 
     allocate(sum_Si_csn_all(nSEDgroups,4))
  ...
  do i=1,npartmax
     ...
     do ii=4,7
		call inp_SED_table(age, Z, 2+2*ii, .true., Si_csn_star(:,ii-3))! [cm^2]
	 end do
     ...
     do ip=1,nSEDgroups
		...
		sum_Si_csn_cpu(ip,:) = sum_Si_csn_cpu(ip,:) + L_star(ip)*Si_csn_star(ip,:)
  ...
  call MPI_ALLREDUCE(sum_csn_cpu, sum_csn_all, nSEDgroups*(nIons),         &
                     MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
  call MPI_ALLREDUCE(sum_Si_csn_cpu, sum_Si_csn_all, nSEDgroups*(4),         &
                     MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
  ...
  do ip=1,nSEDgroups
     ...
     if(sum_L_all(ip) .gt. 0.) then
		...
		SiCSN(ip,:) = sum_Si_csn_all(ip,:) / sum_L_all(ip)
  ...
  open(unit=100,file='SiCSN.dat',status='replace',form='formatted',action='write')
  do ip=1,nSEDgroups
	write(100,910) SiCSN(ip,:)
  end do
  close(100)
  910 format ('', 20(1pe12.3))
